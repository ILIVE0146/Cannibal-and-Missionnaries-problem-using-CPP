...
#include <vector>
#include <set>
int nextEmptyPositionInGraph = 0;
enum List
{
    zero,
    one,
...
Node createNode(Boatposition bp, std::pair<List, List> left, std::pair<List, List> right, const std::vector<Node> &graphNodes)
{
    Node tempNode;
    ...
    if (isMatched == false)
    {
        tempNode.position = nextEmptyPositionInGraph;
        nextEmptyPositionInGraph++;
    }
    else
    {
